...
class MqttClient {
  ...
  constructor(roomId: string, deviceId: string, config: MqttConfig) {
    ...
    this.logger.debug(this.formatLogMessage('INITIALIZING_CLIENT', {
  ...
  private setupListeners() {
    this.client.on('connect', () => {
      ...
      topics.forEach(topic => {
        this.client.subscribe(topic, { qos: 1 }, (err, granted) => {
          if (err) {
            this.logger.error(this.formatLogMessage('SUBSCRIBE_ERROR', {
              topic,
              error: err.message
            }));
          } else {
            ...
            if (topic.endsWith('/status')) {
              this.logger.log(this.formatLogMessage('STATUS_SUBSCRIBE_SUCCESS', { 
                topic,
                granted: granted 
              }));
            }
    ...
    this.client.on('message', (topic: string, message: Buffer) => {
      try {
        ...
        if (messageType === 'status') {
          ...
          this.logger.debug(this.formatLogMessage('CONTROL_MESSAGE_RECEIVED', {
            topic,
    ...
    this.client.on('packetsend', (packet: mqtt.Packet) => {
      if ('topic' in packet && typeof packet.topic === 'string' && packet.topic.endsWith('/status')) {
        this.logger.debug(this.formatLogMessage('STATUS_PACKET_SENT', {
          topic: packet.topic,
    ...
    this.client.on('packetreceive', (packet: mqtt.Packet) => {
      if ('topic' in packet && typeof packet.topic === 'string' && packet.topic.endsWith('/status')) {
        this.logger.debug(this.formatLogMessage('STATUS_PACKET_RECEIVED', {
          topic: packet.topic,
          packetType: packet.cmd
        }));
      }
    ...
    this.client.on('error', (error) => {
      this.logger.error(this.formatLogMessage('CONNECTION_ERROR', {
        error: error.message
      }));
    });

    this.client.on('close', () => {
      this.logger.warn(this.formatLogMessage('CONNECTION_CLOSED', {
        clientId: this.config.clientId
      }));
    });

    this.client.on('offline', () => {
      this.logger.warn(this.formatLogMessage('CLIENT_OFFLINE', {
        clientId: this.config.clientId
      }));
    });

    this.client.on('reconnect', () => {
      this.logger.log(this.formatLogMessage('RECONNECTING', {
        brokerUrl: this.brokerUrl,
        clientId: this.config.clientId
      }));
    });
  ...
  private handleStatusUpdate(status: any) {
    this.logger.log(this.formatLogMessage('STATUS_UPDATE', {
      roomId: this.roomId,
      deviceId: this.deviceId,
      status,
      timestamp: new Date().toISOString()
  ...
  public publishStatus(status: any) {
    ...
    this.logger.debug(this.formatLogMessage('PUBLISHING_STATUS', {
      topic,
      status: message
    }));
    ...
    this.client.publish(topic, JSON.stringify(message), { qos: 1 }, (error) => {
      if (error) {
        this.logger.error(this.formatLogMessage('PUBLISH_ERROR', {
          topic,
          error: error.message
        }));
      } else {
  ...
  public disconnect() {
    if (this.client) {
      this.logger.log(this.formatLogMessage('DISCONNECTING', {
        clientId: this.config.clientId
      }));
      this.client.end();
    }
  ...
  private publishDeviceInfo() {
    ...
    this.logger.debug(this.formatLogMessage('PUBLISHING_DEVICE_INFO', {
      topic,
      deviceInfo
    }));
    ...
    this.client.publish(topic, JSON.stringify(deviceInfo), { qos: 1 }, (error) => {
      if (error) {
        this.logger.error(this.formatLogMessage('PUBLISH_DEVICE_INFO_ERROR', {
          error: error.message
        }));
      } else {
        this.logger.log(this.formatLogMessage('DEVICE_INFO_PUBLISHED', {
          topic,
          deviceInfo
        }));
      }
  ...
  public checkSubscriptions() {
    ...
    this.logger.debug(this.formatLogMessage('CHECKING_SUBSCRIPTIONS', {
      topics,
      clientId: this.config.clientId
    }));
    ...
    topics.forEach(topic => {
      if (this.activeSubscriptions.has(topic)) {
        this.logger.log(this.formatLogMessage('SUBSCRIPTION_ACTIVE', { 
          topic,
          qos: 1
        }));
      } else {
        ...
        this.client.subscribe(topic, { qos: 1 }, (err) => {
          if (err) {
            this.logger.error(this.formatLogMessage('RESUBSCRIBE_ERROR', {
              topic,
              error: err.message
            }));
          } else {
    ...
    this.logger.debug(this.formatLogMessage('CURRENT_SUBSCRIPTIONS', {
  ...
  public unsubscribe(topic: string) {
    this.client.unsubscribe(topic, (err) => {
      if (err) {
        this.logger.error(this.formatLogMessage('UNSUBSCRIBE_ERROR', {
          topic,
          error: err.message
        }));
      } else {
    ...
    process.stdin.on('data', (key: Buffer | string) => {
      ...
      switch(keyStr) {
        case 'r': {
          ...
          this.logger.debug(this.formatLogMessage('SENDING_CONTROL_COMMAND', {
            topic,
          ...
          this.client.publish(topic, JSON.stringify(controlMessage), { qos: 1 }, (error) => {
            if (error) {
              this.logger.error(this.formatLogMessage('CONTROL_PUBLISH_ERROR', {
                error: error.message
              }));
            } else {
              this.logger.debug(this.formatLogMessage('CONTROL_COMMAND_SENT', {
                topic,
...
async function main() {
  ...
  try {
    const client = new MqttClient(
      '4cb35a52-8a15-48fb-98f8-fd74a08e8939',
      '18e00eb9-d650-4df7-a282-c647e4e06ae2',
      config
    );
